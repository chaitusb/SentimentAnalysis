...
public class perceptron_presence {
	...
	public static void weightVectorCalculation(
		...
		for (int i = 0; i < wordlist.size(); i++) {
			weightVector[0][i] = 0.0;// randomGenerator.nextDouble();
		}
		int mistakes = 1;
		while (mistakes != 0) {
			loopCounter++;
			...
			for (int k = 0; k < 4; k++) {
				for (int i = 0; i < 200; i++) {
					...
					for (int j = 0; j < wordlist.size(); j++) {
						if (postrainingMatrix.get(k)[i][j] > 0) {
							weightedSum += 1 * weightVector[0][j];
						}
					}
					if (weightedSum < 0) {
						mistakes++;
						for (int j = 0; j < wordlist.size(); j++) {
							if (postrainingMatrix.get(k)[i][j] > 0) {
								weightVector[0][j] = weightVector[0][j] + 1;
							}
						}
					}
				}
				for (int i = 0; i < 200; i++) {
					double weightedSum = 0.0;
					for (int j = 0; j < wordlist.size(); j++) {
						if (negTrainingMatrix.get(k)[i][j] > 0) {
							weightedSum += 1 * weightVector[0][j];
						}
					}
					if (weightedSum >= 0) {
		...
		for (int i = 0; i < 200; i++) {
			...
			for (int j = 0; j < wordlist.size(); j++) {
				if (testPosMatrix[i][j] > 0) {
					weightedsum += weightVector[0][j] * 1;
				}
			}
			if (weightedsum < 0) {
				misclassification++;
